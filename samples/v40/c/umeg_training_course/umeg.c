...
#include <dsa40.h> /* EDI high level library version 2.0x.  */
...
int main(int argc, char *argv[])
{
     ...
     * object is defined in the dsa30 library. The client does not need to access
...
#define AXIS_NB_Y 1
     ...
     * object. This hidden object is also defined in the dsa30 library, and can be
    ...
    /*----------------------------------------------------------------------------------*/
    /* Hole 1                                                                           */
    /* Chapter "Creating the objects"       											*/
    /* Create axisX, axisY and ultimet objects here										*/
    /*----------------------------------------------------------------------------------*/
    ...
    /* Hole 2                                                                           */
    /* Chapter "Creating the objects"       											*/
    /* Create interpolation group objects here			    							*/
    /*----------------------------------------------------------------------------------*/
    ...
    /* Hole 3                                                                           */
    /* Chapter "Creating the objects"       											*/
    /* Add the controllers to the created group here									*/
    /*----------------------------------------------------------------------------------*/

    /* Now associate the ULTIMET to the interpolation group. When you ask for an
     * interpolated movement on a group of axes, the dsa30 library will use the ULTIMET
     * associated with the group.
     */
    /*----------------------------------------------------------------------------------*/
    /* Hole 4                                                                           */
    /* Chapter "Creating the objects"       											*/
    /* Associate the ultimet object to the interpolation group here						*/
    /*----------------------------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------------------------*/
    /* Hole 5                                                                           */
    /* Chapter "Establishing the communication"                   						*/
    /* Open the communication on both controller (Nr 0 and Nr 1) and on ultimet (*) here	*/
    /*----------------------------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------------------------*/
    /* Hole 6                                                                           */
    /* Chapter "Powering on (and off)"													*/
    /* Power on the both controller separately, here									*/
    /*----------------------------------------------------------------------------------*/
    ...
    */
    ...
    /* Hole 7                                                                           */
    /* Chapter "Homing" 																*/
    /* Perform the home procedure on the both controller using the group, here			*/
    /*----------------------------------------------------------------------------------*/
    ...
    /* Now, before we can make other movements, we must wait until the homing procedure is terminated. To this
    ...
    /*
     * We will start the thread once controllers are homed.
     ...
     */
    {
        double profileSpeed = 0.01; /* m/s (or turns/secs) for rotary motors */
        double profileAcc   = 10.0; /* m/s^2 or rad/s^2 */
        double jerkTime     = 0.01; /* seconds */

        /*----------------------------------------------------------------------------------*/
        /* Hole 8                                                                           */
        /* Chapter "Simple movements"														*/
        /* Define profile velocity, profile acceleration and jerk time, here				*/
        /*----------------------------------------------------------------------------------*/
...

    {
        ...
        /* Hole 9                                                                           */
        /* Chapter "Simple movements"														*/
        /* Go immediately to the targetPositionX position on the X controller, here 		*/
        /*----------------------------------------------------------------------------------*/
        ...
        /* Hole 10                                                                          */
        /* Chapter "Simple movements"														*/
        /* Wait the end of the movement on the both controllers								*/
        /*----------------------------------------------------------------------------------*/
    }
    ...
    /*------------------------------------------------------------------------------------------*/
    /* Hole 20                                                                                   */
    /* Chapter "Interpolated movements"															*/
    /* Start interpolation mode, here															*/
    /*------------------------------------------------------------------------------------------*/
     ...
     */
    {
        ...
        /* Hole 21                                                                                  */
        /* Chapter "Interpolated movements"															*/
        /* Start the trajectory with a linear segment to the defined position, here					*/
        /*------------------------------------------------------------------------------------------*/
     ...
     */
    {
        ...
        /* Hole 22                                                                                  */
        /* Chapter "Interpolated movements"															*/
        /* Enable concatenation for following segment, here											*/
        /*------------------------------------------------------------------------------------------*/
    ...
    /* Hole 23                                                                                  */
    /* Chapter "Interpolated movements"															*/
    /* Wait that interpolation movement is finished, here										*/
    /*------------------------------------------------------------------------------------------*/
     ...
     */
    {
        ...
        /*------------------------------------------------------------------------------------------*/
        /* Hole 12                                                                                  */
        /* Chapter "Access to the controller parameters"											*/
        /* Get value of user register X2, here														*/
        /*------------------------------------------------------------------------------------------*/
        ...
        /* Hole 13                                                                                  */
        /* Chapter "Access to the controller parameters"											*/
        /* Write user register X2 with incremented value, here										*/
        /*------------------------------------------------------------------------------------------*/
    ...
    /*------------------------------------------------------------------------------------------*/
    /* Hole 14                                                                                  */
    /* Chapter "Saving and resetting"															*/
    /* Save  all parameters in permanent storage, here											*/
    /*------------------------------------------------------------------------------------------*/
     ...
     */
    {
        ...
        /*------------------------------------------------------------------------------------------*/
        /* Hole 15                                                                                  */
        /* Chapter "Status handling"	    														*/
        /* Wait that controller is present again, here	    										*/
        /*------------------------------------------------------------------------------------------*/
    }
    ...
    /*------------------------------------------------------------------------------------------*/
    /* Hole 18                                                                                  */
    /* Chapter "Asynchronous function calls"													*/
    /* Wait that homing move is ended asynchronously. On end of move, handler "callback" should */
    /* be called, passing "2" as parameter                                                      */
    /*------------------------------------------------------------------------------------------*/
     ...
     * -connection to the ULITMET:
     */
    /*------------------------------------------------------------------------------------------*/
    /* Hole 16                                                                                  */
    /* Chapter "Terminating the application"													*/
    /* Close ultimet communication, here														*/
    /*------------------------------------------------------------------------------------------*/
    ...
    /* Hole 17                                                                                  */
    /* Chapter "Terminating the application"													*/
    /* Close controllers communication, here													*/
    /*------------------------------------------------------------------------------------------*/
    ...
    /* Hole 19                                                                                  */
    /* Chapter "Terminating the application"													*/
    /* Destroy all created objects (group, ultimet, controllers), here							*/
    /*------------------------------------------------------------------------------------------*/
...
static void display_thread(void *param)
{
    ...
    for (;;) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            /*------------------------------------------------------------------------------------------*/
            /* Hole 11                                                                                  */
            /* Chapter "Monitoring datas"	            												*/
            /* Use specific function to get actual position of each axis and store them in "pos" table  */
            /*------------------------------------------------------------------------------------------*/
        }
        ...
        printf("ULTIMET: %c%c%c, ",
               ultimet_status.ultimet.moving ? 'M' : '-',
               ultimet_status.ultimet.warning ? 'W' : '-',
               ultimet_status.ultimet.error ? 'E' : '-');
